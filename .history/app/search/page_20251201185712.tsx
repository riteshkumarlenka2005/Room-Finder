...
export default function SearchPage() {
  ...
  const filteredRooms = rooms.filter((room) => {
    // Search query filter (title OR location OR fullAddress)
    if (searchQuery) {
    ...
    if (selectedFilters.priceRange) {
      const [minStr, maxStr] = selectedFilters.priceRange.split("-")
    ...
    if (selectedFilters.sharing.length > 0) {
      // check any match (owner provided sharing string may vary)
      const matchesSharing = selectedFilters.sharing.some((s) => {
        return room.sharing.toLowerCase().includes(s.toLowerCase())
      })
      if (!matchesSharing) return false
    }

    // Amenities filter: ensure room.features contains all selected amenities
    if (selectedFilters.amenities.length > 0) {
      // normalize room.features to lower-case strings safely
      const roomFeaturesNormalized = (room.features || []).map((f: any) => String(f).toLowerCase())
      const hasAll = selectedFilters.amenities.every((a) => roomFeaturesNormalized.includes(a.toLowerCase()))
      if (!hasAll) return false
    }

    // Location filter (if used via filter panel)
    if (selectedFilters.location) {
      const loc = selectedFilters.location.toLowerCase()
      if (!room.location.toLowerCase().includes(loc) && !room.fullAddress.toLowerCase().includes(loc)) {
        return false
      }
    }
