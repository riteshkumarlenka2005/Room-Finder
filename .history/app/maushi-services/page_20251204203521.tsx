...
export default function MaushiServicesPage() {
   ...
   * - If it's a storage path / filename -> use supabase.storage.getPublicUrl for `helper-images` bucket
  ...
  const resolveImageUrl = (path: any) => {
    ...
    // if path looks like "helper-images/..." or starts with bucket/
    // attempt to build public URL via supabase storage
    try {
      // Supabase JS may return different shapes; handle accordingly
      const res: any = supabase.storage.from("helper-images").getPublicUrl(s)
      // v1 shape: { publicURL: '...' } ; v2: { data: { publicUrl } }
      const publicUrl = res?.data?.publicUrl ?? res?.publicURL ?? res?.publicUrl ?? null
      if (publicUrl) return publicUrl
    } catch (e) {
      // ignore and fallback to direct path
    }

    // Common case DB stored just filename (like "abc.jpg") or "folder/abc.jpg"
    // Supabase public URL format (cdn-like) if bucket is public:
    // We'll attempt to form a URL using the project URL if available from client.
    // But safer fallback: return the raw path (sometimes works if user stored full path),
    // or placeholder if it obviously isn't usable.
  ...
  async function fetchHelpers() {
    ...
    const normalized = (data || []).map((h: any) => {
      ...
      const parsedImages = Array.isArray(rawImages) ? rawImages.map((x) => (typeof x === "string" ? x : JSON.stringify(x))) : (typeof rawImages === "string" ? [rawImages] : [])

      const rawFood = parseMaybeJSON(h.food_images ?? h.food_images_json ?? h.foods)
      const parsedFood = Array.isArray(rawFood) ? rawFood.map((x) => (typeof x === "string" ? x : JSON.stringify(x))) : (typeof rawFood === "string" ? [rawFood] : [])
      ...
      // experience
      const experience_years = h.experience_years ?? h.experience ?? h.years ?? ""

      return {
        // keep original fields too for any other usage
        ...h,
        ...
        salaryMax,
        experience_years,
        other_skills: h.other_skills ?? h.otherSkills ?? h.skills ?? "",

        // images arrays (not yet resolved to full URLs)
        images: parsedImages,
        food_images: parsedFood,
        profile_photo: profilePhoto,

        // convenient helpers
        _getMainImageUrl: () => {
          // priority: food_images -> images -> profile_photo -> placeholder
          if (Array.isArray(parsedFood) && parsedFood.length > 0) return resolveImageUrl(parsedFood[0])
          if (Array.isArray(parsedImages) && parsedImages.length > 0) return resolveImageUrl(parsedImages[0])
          if (profilePhoto) return resolveImageUrl(profilePhoto)
          return "/placeholder.svg"
        },

        _getAnyImageUrls: () => {
          const arr: string[] = []
          if (Array.isArray(parsedFood) && parsedFood.length > 0) arr.push(...parsedFood.map(resolveImageUrl))
          if (Array.isArray(parsedImages) && parsedImages.length > 0) arr.push(...parsedImages.map(resolveImageUrl))
          if (profilePhoto) arr.push(resolveImageUrl(profilePhoto))
