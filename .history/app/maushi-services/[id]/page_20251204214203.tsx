...
export default function HelperDetailsPage() {
  ...
  useEffect(() => {
    ...
    const fetchHelper = async () => {
      ...
      // ---- Specialties ----
      const specialtiesJSON = parseMaybeJSON(data.specialties);
      const dishesJSON = parseMaybeJSON(data.dishes);
      const cuisine = data.cuisine_type ? [data.cuisine_type] : [];
      ...
      // ---- Services ----
      const servicesJSON = parseMaybeJSON(data.services);
      const preferredWork = parseMaybeJSON(data.preferred_work);
      ...
      // ---- Working Hour ----
      const workingHours = parseMaybeJSON(data.working_hour);

      // ---- Nearby ----
      const nearby = parseMaybeJSON(data.nearby_places);
      ...
      const normalized = {
        ...
        experience: data.experience_ye ?? data.experience ?? "0",
        ...
        images: finalImages.length ? finalImages : ["/placeholder.svg"],
        foodImages,
        profilePhoto,
