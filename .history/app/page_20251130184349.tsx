...
export default function HomePage() {
  ...
  // --- add/useEffect import at top of file ---
// import { useState, useEffect } from "react"

const [featuredRooms, setFeaturedRooms] = useState<any[]>([]);

// fetch featured rooms from your API on mount
useEffect(() => {
  async function fetchFeatured() {
    try {
      const res = await fetch("/api/properties");
      const data = await res.json();
      // adjust if your API shape differs; this assumes `data.properties` is array
      setFeaturedRooms(data?.properties || []);
    } catch (error) {
      console.error("Failed to load featured rooms", error);
      setFeaturedRooms([]); // fail-safe
    }
  }
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div className="grid md:grid-cols-3 gap-8">
      ...
      {featuredRooms.map((room) => {
        ...
        // ✔ Room ID (API returns property_id OR id)
...
<div className="flex flex-wrap gap-1 mb-4">
  ...
  {/* FIX: define features inside map function */}
  {(() => {
