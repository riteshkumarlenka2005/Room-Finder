...
useEffect(() => {
  async function fetchFeatured() {
    try {
      ...
      const cleaned = (properties as any[]).map((p: any) => ({

        id: p.id,
        title: p.title,

        price: Number(p.price || p.monthly_rent || 0),

        location:
          p.full_address ||
          [p.area, p.city, p.district, p.state].filter(Boolean).join(", ") ||
          "Not provided",

        type: p.property_type || p.bhk || "Room",
        owner: p.owner_name || "Unknown Owner",

        features: [
          ...(Array.isArray(p.amenities) ? p.amenities : []),
          ...(Array.isArray(p.furniture) ? p.furniture : []),
        ],
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div className="grid md:grid-cols-3 gap-8">
      {featuredRooms.map((room) => {
        ...
        const firstImage = images.length > 0 ? images[0] : "/placeholder.svg";
