...
export default function ListPropertyPage() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      // get session at time of submit - ensures token available
      const { data } = await supabase.auth.getSession();
      const session = (data as any).session;
      const token = session?.access_token;

      if (!token) {
        alert("You must be logged in to submit. Token not found.");
        return;
      }
      ...
      if (!res.ok) {
        alert("Error: " + (dataResp?.error || "Unknown error"));
        return;
      }
                  ...
                  <div className="grid md:grid-cols-3 gap-3 mt-3">
                    {furnitureList.map((furn) => (
                      <div key={furn} className="flex items-center space-x-2">
                        <Checkbox
                          id={`furniture-${furn}`}
                          checked={formData.furniture.includes(furn)}
                          onCheckedChange={() => handleArrayToggle("furniture", furn)}
                        />
                        <Label htmlFor={`furniture-${furn}`} className="text-sm">
                          {furn}
                        </Label>
                      </div>
                    ))}
